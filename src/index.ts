...
const TAILWIND_ETAG = createHash('sha1')
  .update(readFileSync(`${__dirname}/../app/tailwind.css.br`))
  .digest('hex');
    ...
    routes: [
      ...
      },
      {
        ...
        dest: (req, res) => {
          ...
          if (
            `${req.headers['accept-encoding'] || ''}`.split(', ').includes('br')
          ) {
            res.setHeader('Content-Encoding', 'br');
            createReadStream(`${__dirname}/../app/tailwind.css.br`).pipe(res);
          } else {
            createReadStream(`${__dirname}/../app/tailwind.css.br`)
...
try {
  for (const filename of readdirSync(
    resolve(`${__dirname}/../snowpack-cache`),
  )) {
    if (filename.endsWith('.js')) {
      alias[filename.replace(/\.js$/, '')] = join(
